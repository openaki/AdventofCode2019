...
fn handle_opcode(program_state: &mut Program) -> OpcodeResult {
    ...
    let get_fn = |program: &mut Vec<i64>, index: usize| -> i64 {
    ...
    let sval = |program: &mut Vec<i64>, param, index| -> usize {
        if param == 1 {
            (pc as i32 + index) as usize
        } else if param == 2 {
            (relative_base as i64 + program[pc + index as usize]) as usize
    ...
    let set_fn = |program: &mut Vec<i64>, index: usize, val: i64| {
        if index >= program.len() {
...
mod tests {
    ...
    fn test_solve2a() {
        assert_eq!(
            2,
            run_program_first_location(&mut Program::new(vec!(1, 0, 0, 0, 99)))
        );
        assert_eq!(
            2,
            run_program_first_location(&mut Program::new(vec!(2, 3, 0, 3, 99)))
        );
        assert_eq!(
            30,
            run_program_first_location(&mut Program::new(vec!(1, 1, 1, 4, 99, 5, 6, 0, 99)))
        );
        assert_eq!(
            3500,
            run_program_first_location(&mut Program::new(vec!(
